...
internal class AiRequestLogRepository(IDbConnectionFactory factory, IUnitOfWork uow) : IAiRequestLogRepository
{
        ...
        int TotalCacheReadInputTokens)
    {
        public AiMetricsTotals ToDomain() => new(
            TotalCostUsd, TotalCalls, SuccessCalls, ErrorCalls, AverageLatencyMs,
            TotalInputTokens, TotalOutputTokens, TotalCacheCreationInputTokens, TotalCacheReadInputTokens);
    }
    ...
    private sealed record AiMetricsTimeBucketRow(
        ...
        int Tokens)
    {
        public AiMetricsTimeBucket ToDomain() => new(Bucket, Provider, CostUsd, Calls, Tokens);
    }
    ...
    private sealed record AiMetricsBreakdownRowSql(
        ...
        int Tokens)
    {
        public AiMetricsBreakdownRow ToDomain() => new(Key, Calls, CostUsd, Tokens);
    }
}
