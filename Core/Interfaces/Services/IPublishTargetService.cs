...
public interface IPublishTargetService
{
	...
	Task<List<PublishTarget>> GetAllByProjectAsync(Guid projectId, CancellationToken cancellationToken = default);
	Task<List<PublishTarget>> GetActiveAsync(CancellationToken cancellationToken = default);
	Task<List<PublishTarget>> GetActiveByProjectAsync(Guid projectId, CancellationToken cancellationToken = default);
	Task<PublishTarget> GetByIdAsync(Guid id, CancellationToken cancellationToken = default);
	Task<PublishTarget> CreateAsync(string name, Platform platform, string identifier, string systemPrompt, Guid projectId, CancellationToken cancellationToken = default);
